...
const SearchNav = () => {
  ...
  const { data: clinicsData, isLoading: loadingClinics } = useGet<Clinic[]>(
  ...
  const { data: doctorsData, isLoading: loadingDoctors } = useGet<Doctor[]>(
    "/doctors/search",
    {
      params: { search: debouncedSearch },
      enabled: debouncedSearch.length >= 3,
                ...
                {results.map((result, index) => (
                  <li key={`${result.type}-${result.id}-${index}`}>
                    <Button
                      ...
                      classes="w-full flex items-center gap-3 p-3 hover:bg-base-200 rounded-lg transition-colors text-left group"
                    >
                      {/* Badge con icona */}
                      <div
                        className={`badge ${result.type === "clinic" ? "badge-primary" : "badge-secondary"} badge-lg shrink-0`}
                      >
                        <span className="text-lg">
                          {result.type === "clinic" ? "🏥" : "👨‍⚕️"}
                        </span>
                      </div>
